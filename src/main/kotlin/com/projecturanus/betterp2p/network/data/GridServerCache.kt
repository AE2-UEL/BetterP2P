...
class GridServerCache(private val grid: IGrid, val player: EntityPlayer, var type: Int) {
  ...
  fun linkP2P(
    ...
    if (input.javaClass != output.javaClass) {
      output =
          changeP2PType(output, BetterP2P.proxy.getP2PFromClass(input.javaClass)!!) ?: return null
    }
    ...
    // Perform the link
    val inputResult: PartP2PTunnel<*> =
        updateP2P(inputIndex, input, frequency, false, input.customInventoryName)
    val outputResult: PartP2PTunnel<*> =
        updateP2P(outputIndex, output, frequency, true, input.customInventoryName)

    // Special case for interfaces
    if (input is IInterfaceHost && output is IInterfaceHost) {
      val drops = mutableListOf<ItemStack>()
      handleInterface(
          input, output, inputResult as IInterfaceHost, outputResult as IInterfaceHost, drops)
  ...
  /**
   * Sets the p2p tunnel to the frequency, output, and custom name. Removes the old one and replaces
   * it, which lets AE2 trigger the Grid refresh for us (though we need to update the tunnels
   * ourselves)
   */
  private fun updateP2P(
      key: P2PLocation,
      tunnel: PartP2PTunnel<*>,
      frequency: Short,
      output: Boolean,
      name: String
  ): PartP2PTunnel<*> {
    ...
    val dir: AEPartLocation =
        tunnel.host?.addPart(newType, side, player, EnumHand.MAIN_HAND)
  ...
  private fun handleInterface(
      oldIn: IInterfaceHost,
      oldOut: IInterfaceHost,
      newIn: IInterfaceHost,
      newOut: IInterfaceHost,
    ...
    upgradesIn.forEachIndexed { index, stack ->
      (newIn.interfaceDuality.getInventoryByName("upgrades") as UpgradeInventory).setStackInSlot(
          index, stack)
    }
    ...
    upgradesOut.forEachIndexed { index, stack ->
      (newOut.interfaceDuality.getInventoryByName("upgrades") as UpgradeInventory).setStackInSlot(
          index, stack)
    }
    ...
    settingsIn.settings.forEach { newIn.configManager.putSetting(it, settingsIn.getSetting(it)) }
    val settingsOut = oldOut.interfaceDuality.configManager
    settingsOut.settings.forEach { newOut.configManager.putSetting(it, settingsOut.getSetting(it)) }
