...
class GuiAdvancedMemoryCard(msg: S2COpenGui) : GuiScreen(), TextureBound {
  ...
  private var type: ClientTunnelInfo? =
      BetterP2P.proxy.getP2PFromIndex(msg.memoryInfo.type) as? ClientTunnelInfo
  ...
  val background: ResourceLocation =
      ResourceLocation(Tags.MODID, "textures/gui/advanced_memory_card.png")
  ...
  init {
    ...
    resizeButton =
        object : WidgetButton(this, 0, 0, 32, 32) {
    ...
    modeButton =
        object : WidgetButton(this, 0, 0, 32, 32) {
          val modeDescriptions: List<List<String>> =
              listOf(
                  fmtTooltips(
    ...
    typeButton =
        object : WidgetButton(this, 0, 0, 32, 32), ITypeReceiver {
          ...
          private val me =
              BetterP2P.proxy.getP2PFromClass(PartP2PTunnelME::class.java) as ClientTunnelInfo
          private val fluid =
              BetterP2P.proxy.getP2PFromClass(PartP2PFluids::class.java) as ClientTunnelInfo
          private val redstone =
              BetterP2P.proxy.getP2PFromClass(PartP2PRedstone::class.java) as ClientTunnelInfo
          ...
          override fun draw(mc: Minecraft, mouseX: Int, mouseY: Int, partial: Float) {
            ...
            if (type != null) {
              drawBlockIcon(
                  mc, type!!.icon(), x = this.x + 2, y = this.y + 2, width = 28.0, height = 28.0)
            } else {
              drawBlockIcon(
                  mc,
                  redstone.icon(),
                  ...
                  height = 18.0)
              drawBlockIcon(
                  mc, fluid.icon(), x = this.x + 7, y = this.y + 7, width = 18.0, height = 18.0)
              drawBlockIcon(
                  mc, me.icon(), x = this.x + 2, y = this.y + 2, width = 18.0, height = 18.0)
            }
          }
          ...
          private fun commitType() {
            if (type == null) {
              ...
              (hoverText as MutableList)[0] =
                  I18n.format(
                      "gui.advanced_memory_card.types.filtered", "§a" + type!!.stack.displayName)
            }
    ...
    //        toAdd = BetterP2P.proxy.getP2PFromClass(PartP2PInterface::class.java) as?
    // ClientTunnelInfo
    //        if (toAdd != null) {
    //            typeSelectorList.add(toAdd)
    //        }
  ...
  override fun initGui() {
    ...
    col = WidgetP2PColumn(fontRenderer, this, infos, 0, 0, ::selectedInfo, ::mode, scrollBar)
  ...
  private fun checkInfo() {
    ...
    //        infos.filtered.groupBy { it.frequency }.filter { it.value.none { x -> !x.output }
    // }.forEach { it.value.forEach { info ->
    //            info.error = true
    //        } }
  ...
  override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
    ...
    fontRenderer.drawString(
        I18n.format("item.advanced_memory_card.name"), guiLeft + tableX, guiTop + 6, 0)
  ...
  private fun drawBackground() {
    ...
    drawTexturedQuad(
        ...
        guiLeft.toDouble(),
        guiTop.toDouble(),
        (guiLeft + GUI_WIDTH).toDouble(),
        guiTop + 60.0,
        u0 = 0.0,
        v0 = 0.0,
        u1 = 1.0,
        v1 = 60.0 / GUI_TEX_HEIGHT)
    ...
    for (i in 0 until scale.size(ySize - 75) - 2) {
      drawTexturedQuad(
          tessellator,
          guiLeft.toDouble(),
          guiTop + 60.0 + p2pHeight * i,
          (guiLeft + GUI_WIDTH).toDouble(),
          guiTop + 60.0 + p2pHeight * (i + 1),
          u0 = 0.0,
          v0 = 60.0 / GUI_TEX_HEIGHT,
          u1 = 1.0,
          v1 = 102.0 / GUI_TEX_HEIGHT)
    }
    ...
    drawTexturedQuad(
        ...
        guiTop + ySize - 98.0,
        (guiLeft + GUI_WIDTH).toDouble(),
        (guiTop + ySize).toDouble(),
        u0 = 0.0,
        v0 = 102.0 / GUI_TEX_HEIGHT,
        u1 = 1.0,
        v1 = 200.0 / GUI_TEX_HEIGHT)
  }
  ...
  override fun keyTyped(typedChar: Char, keyCode: Int) {
    ...
    if (!(typedChar.isWhitespace() && searchBar.text.isEmpty()) &&
  ...
  public override fun drawHoveringText(
