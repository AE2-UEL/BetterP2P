...
class WidgetP2PColumn(
  ...
  fun finishRename() {
    ...
    if (renameBar.info != null &&
        renameBar.text.isNotEmpty() &&
        renameBar.info!!.name != renameBar.text) {
  ...
  fun findInput(frequency: Short?) = infos.filtered.find { it.frequency == frequency && !it.output }

  fun findOutput(frequency: Short?) = infos.filtered.find { it.frequency == frequency && it.output }
  ...
  fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
    ...
    for ((index, widget) in entries.withIndex()) {
      ...
      } else if (mouseX > widget.x.toDouble() + 50 &&
          mouseX < widget.x.toDouble() + 50 + 160 &&
          mouseY > widget.y.toDouble() + 1 &&
          mouseY < widget.y.toDouble() + 1 + 13 &&
      ...
      } else if (mouseX > widget.x &&
          mouseX < widget.x + P2PEntryConstants.WIDTH &&
          mouseY > widget.y &&
          mouseY < widget.y + P2PEntryConstants.HEIGHT &&
          info != null) {
  ...
  fun mouseHovered(mouseX: Int, mouseY: Int) {
    entries.forEach {
      ...
      if (mouseX > it.x.toDouble() &&
          mouseX < it.x + 20.0 &&
          mouseY > it.y.toDouble() &&
          mouseY < it.y + P2PEntryConstants.HEIGHT &&
          info != null) {
        gui.drawHoveringText(info.hoverInfo, it.x + 168, it.y + 15, gui.mc.fontRenderer)
      } else if (mouseX > it.x.toDouble() + 50 &&
          mouseX < it.x.toDouble() + 50 + 160 &&
          mouseY > it.y.toDouble() + 1 &&
          mouseY < it.y.toDouble() + 1 + 13 &&
          !renameBar.isFocused &&
          info != null) {
        GuiTextField.drawRect(
            it.x + 50, it.y + 1, it.x + 50 + 160, it.y + 1 + 12, 0x6E000000 // ARGB xd
            )
      }
    }
  }
