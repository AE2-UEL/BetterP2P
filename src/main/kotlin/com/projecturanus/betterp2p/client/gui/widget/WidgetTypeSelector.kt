...
/** Select a type. */
class WidgetTypeSelector(var x: Int, var y: Int, val p2pTypes: List<ClientTunnelInfo>) : Widget() {
  var visible = false
  val width: Int
  val height: Int
  ...
  fun render(gui: GuiAdvancedMemoryCard, mouseX: Int, mouseY: Int, partialTicks: Float) {
    ...
    for ((i, type) in p2pTypes.withIndex()) {
      ...
      val iconHover =
          mouseX > iconPosX && mouseX < iconPosX + 18 && mouseY > iconPosY && mouseY < iconPosY + 18
      if (iconHover) {
        hoveredIdx = i
        GuiScreen.drawRect(iconPosX, iconPosY, iconPosX + 18, iconPosY + 18, 0xFF00FF00.toInt())
      }
      drawBlockIcon(gui.mc, type.icon(), x = iconPosX + 1, y = iconPosY + 1)
    }
    if (useAny) {
      ...
      drawBlockIcon(
          gui.mc,
          ResourceLocation("minecraft", "textures/blocks/coal_block.png"),
          x = iconPosX + 1,
          y = iconPosY + 1)
